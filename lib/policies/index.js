...
router.get("/:id", (req, res, next) => {
  ...
  if (!policyId) {
    ...
    return next(new APIError({statusCode: 400, detail: "No policy ID specified"}))
  }

  dataControllerPoliciesTable.get(policyId).default({}).run(req._rdbConn)
    .then(policy => {
      if (!policy["id"]) return next(new APIError({statusCode: 404, detail: "Policy does not exist / You're not authorized"}))
  ...
  dataControllerPoliciesTable.get(policyId).update(policy).run(req._rdbConn)
    .then(updateResult => {
      if (updateResult["skipped"] > 0 &&
        ...
        return next(new APIError({statusCode: 404, detail: "Policy does not exist / You are not authorized"}))
      }
